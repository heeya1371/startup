...
// =================================================================
// 3. LocalStorage를 활용한 익명 후기 저장 및 출력 로직 (백엔드 대체)
// =================================================================
var reviewForm = document.getElementById('review-form');
var reviewTimeline = document.getElementById('review-timeline');

// 로컬스토리지에서 기존 후기를 읽어와 화면에 뿌려주는 함수
function renderReviews() {
    var savedReviews = JSON.parse(localStorage.getItem('campus_reviews')) || [];
    
    if (savedReviews.length === 0) {
        reviewTimeline.innerHTML = '<li style="text-align:center; color:#aaa; list-style:none;">아직 등록된 후기가 없습니다.</li>';
        return;
    }

    // 최신 후기가 위로 오도록 역순 정렬하여 출력
    var htmlContent = savedReviews.map(function(rev) {
        return '<li>' +
    ...
    reviewTimeline.innerHTML = htmlContent;
}

// 후기 등록 버튼(Form Submit) 이벤트 처리
if (reviewForm) {
    reviewForm.addEventListener('submit', function(e) {
        ...
        var currentReviews = JSON.parse(localStorage.getItem('campus_reviews')) || [];
